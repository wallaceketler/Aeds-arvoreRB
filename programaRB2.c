...
void insertTree(No **raiz,No **pai, Record r)
{	
    ...
    {	
        ...
        *raiz=(No*)malloc(sizeof(No));
        ...
        if(cont2>2)
        {
      		//printf("%f" ,(*raiz)->pai->reg.key);
      	    //printf("%f" ,(*raiz)->pai->pai->reg.key);
      	}
    }
    else
    {
	    if(r.key > (*raiz)->reg.key)
	    {
	        ...
	        insertTree(&(*raiz)->dir, raiz, r);
	    }
	    if(r.key < (*raiz)->reg.key)
	    {
	        insertTree(&(*raiz)->esq, raiz, r);
	    }
	}
}
...
void insertRB(No **raiz, No **pai, Record r, No *aux)
{
	No *tio = NULL;
	No *item = NULL;
	...
	if((*raiz)->reg.key==r.key && global!=2)
	{
		...
		global=2;
		insertRB(&(*raiz), pai, r, aux);

	}
	else
	{
		if(r.key > (*raiz)->reg.key)
			insertRB(&(*raiz)->dir, pai, r,aux);
		if(r.key < (*raiz)->reg.key)
			insertRB(&(*raiz)->esq, pai, r,aux);
	}
		...
		{		
			...
			if(cont>2)
			{
				...
				printf("\nTeste1\n");
				while(((((*raiz)->color)!=(aux)->color)) && (*raiz)->pai->color==true)
				{

					printf("\nTeste2\n");
					if((*raiz)->pai->color == (*raiz)->pai->pai->esq->color )
					{	
						printf("\nTeste3\n");
						...
						else if((*raiz)->color==(*raiz)->pai->dir->color)
						{
							(*raiz)=(*raiz)->pai;
							rotacaoEsq(&(*raiz));
						}
					...
					else if((*raiz)->pai->color != (*raiz)->pai->pai->esq->color)
					{
						printf("Teste3");
						...
						else if((*raiz)->color==(*raiz)->pai->esq->color)
						{
							*raiz=(*raiz)->pai;
							rotacaoEsq(&(*raiz));
						}
